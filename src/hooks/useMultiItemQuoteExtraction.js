...
export function useMultiItemQuoteExtraction(apiKey, buyingIntents = []) {
  ...
  const processFile = useCallback(async (file) => {
    ...
    try {
      ...
      setSupplierFields({
        ...
        supplierPhone: getValue(result.supplierPhone) || '',
        supplierAddress: getValue(result.supplierAddress) || '',
  ...
  const processText = useCallback(async (text) => {
    ...
    try {
      ...
      // Pre-fill supplier fields
      setSupplierFields({
        supplierName: getValue(result.supplierName) || getValue(result.supplier) || '',
        supplierContact: getValue(result.supplierContact) || getValue(result.contact) || '',
        supplierEmail: getValue(result.supplierEmail) || getValue(result.email) || '',
        supplierPhone: getValue(result.supplierPhone) || getValue(result.phone) || '',
        supplierAddress: getValue(result.supplierAddress) || getValue(result.address) || '',
        currency: getValue(result.currency) || 'USD',
        incoterm: getValue(result.incoterm) || '',
        quoteDate: getValue(result.quoteDate) || '',
        validUntil: getValue(result.validUntil) || '',
